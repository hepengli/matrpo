...
xdata = np.arange(500)
models = ['central','matrpo','trpo', 'fdmarl']
results = []
for model in models:
    results.append(get_training_curve('collector', model, num_agents=12))
...
sns.tsplot(time=xdata, data=results[3], color='y', linestyle='-', legend=True)
plt.legend(labels=['Central TRPO', 'MATRPO', 'Independent TRPO', 'FDMARL'])
plt.show()
